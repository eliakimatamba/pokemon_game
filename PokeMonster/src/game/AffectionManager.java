...
public class AffectionManager {
    ...
    public String increaseAffection(Pokemon pokemon, int point) {
        int affectionTemp = affectionPoints.get(pokemon);
        affectionTemp += point;
        if (affectionTemp>=50){
            pokemon.addCapability(Status.LIKETRAINER);
        }
        else if (affectionTemp>= 75){
            pokemon.addCapability(Status.FOLLOWTRAINER);
        }
        affectionPoints.replace(pokemon,affectionTemp);
        return "Increased affection level of" + pokemon.getPokeName() + " by " + point
                + " Current AP is " + affectionPoints.get(pokemon);
    }
    ...
    public String decreaseAffection(Pokemon pokemon, int point) {
        ...
        affectionTemp -= point;
        if (affectionTemp<=50){
            pokemon.addCapability(Status.DISLIKETRAINER);
        }
        else if (affectionTemp<=50){
            pokemon.removeCapability(Status.LIKETRAINER);
        }
        else if (affectionTemp<= 75){
            pokemon.removeCapability(Status.FOLLOWTRAINER);
        }
        affectionPoints.replace(pokemon,affectionTemp);
        return "Decreased affection level of" + pokemon.getPokeName() + " by " + point
                + " Current AP is " + affectionPoints.get(pokemon);
    }
